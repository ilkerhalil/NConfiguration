...
namespace NConfiguration.Variables
{
	public class ValueRenderer
	{
		...
		public string Render(VariableStorage variableStorage)
		{
			if (_tokenList.Count == 1)
			{
				var token = _tokenList[0];
				return token.IsVariable ? variableStorage.Get(token.Text) : token.Text;
			}

			var builder = new StringBuilder();
			foreach (var token in _tokenList)
				builder.Append(token.IsVariable ? variableStorage.Get(token.Text) : token.Text);
